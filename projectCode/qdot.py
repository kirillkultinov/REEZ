...
# Modified by Kirill Kultinov on 04/28/2018
    ...
    def parse(self):
        ...
        while s:
            ...
            if op == "c":
                ...
                w = s.read_number()
                t = s.read_text()
                self.handle_text(x, y, j, w, t) # used for handling text (edge text & node text)
            elif op == "E":
                x0, y0 = s.read_point()
                w = s.read_number()
                h = s.read_number()
    ...
    def parse_attrs(self):
        ...
        while self.lookahead.type == LSQUARE:
            ...
            while self.lookahead.type != RSQUARE:
                ...
                attrs[name] = value
                #print("attr name is " + str(name)) #attr name is not affecting the graph
...
class XDotParser(DotParser):
    ...
    def handle_graph(self, attrs):
        ...
        for attr in ("_draw_", "_ldraw_", "_hdraw_", "_tdraw_", "_hldraw_", "_tldraw_"):
            if attr in attrs:
                self.parser = XDotAttrParser(self, attrs[attr])
                self.shapes.extend(self.parser.parse())
    ...
    def handle_node(self, id, attrs):
        ...
        w = float(attrs['width']) * 30
        h = float(attrs['height']) * 30
        shapes = []
